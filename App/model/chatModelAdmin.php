...
class Chat{
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		foreach($userChat as $chat){
			...
			if($chat["sender_userid"] == $from_user_id) {
				...
				$conversation .= '<img width="32px" height="22x" src=".'.$fromUserAvatar.'" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img width="32px" height="22px" src=".'.$toUserAvatar.'" alt="" />';
			}			
	...
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		foreach ($userDetails as $user) {
			$toUserAvatar = $user['imgUsuario'];
			$userSection = '<img src=".'.$user['imgUsuario'].'" alt="" />
				<h5 style="padding-left: 50px; padding-top: 16px;">'.$user['nombreUsuario'].'</h5>
	...
	public function fetchIsTypeStatus($userId){
		...
		WHERE idUsuario = '".$userId."' ORDER BY last_activity DESC LIMIT 1"; 
